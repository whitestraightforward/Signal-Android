...
import dev.chat.fork.messenger.jobs.DeprecatedNotificationJob
...
class MiscellaneousValues internal constructor(store: KeyValueStore) : SignalStoreValues(store) {
  ...
  /**
   * Bypasses the timeout in [dev.chat.fork.messenger.jobs.PreKeysSyncJob] since otherwise we can hit a race.
   */
  ...
  /**
   * The last time we ran an account consistency check via [dev.chat.fork.messenger.jobs.AccountConsistencyWorkerJob]
   */
  ...
  /**
   * The last time we prompted the user regarding a [dev.chat.fork.messenger.util.ConnectivityWarning].
   */
   ...
   * [dev.chat.fork.messenger.service.webrtc.CallingAssets.CURRENT_VERSION] to determine
   ...
   * [dev.chat.fork.messenger.conversation.ConversationArgs.shareDataTimestamp]. Any share whose timestamp is at or
