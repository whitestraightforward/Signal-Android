...
import dev.chat.fork.messenger.database.model.IssuePriority
import dev.chat.fork.messenger.util.Environment.Calling.defaultSfuUrl
...
class InternalValues internal constructor(store: KeyValueStore) : SignalStoreValues(store) {
  ...
  /**
   * Show detailed recipient info in the [dev.chat.fork.messenger.components.settings.conversation.InternalConversationSettingsFragment].
   */
  ...
  /**
   * The minimum [IssuePriority] that an issue recorded by [dev.chat.fork.messenger.database.IssueReporter] must have
