...
import dev.chat.fork.messenger.jobs.MultiDeviceProfileContentUpdateJob
...
class AppRegistrationNetworkController(
  ...
  override suspend fun setProfile(
    ...
    SignalStore.phoneNumberPrivacy.phoneNumberDiscoverabilityMode = if (discoverableByPhoneNumber) {
      dev.chat.fork.messenger.keyvalue.PhoneNumberPrivacyValues.PhoneNumberDiscoverabilityMode.DISCOVERABLE
    } else {
      dev.chat.fork.messenger.keyvalue.PhoneNumberPrivacyValues.PhoneNumberDiscoverabilityMode.NOT_DISCOVERABLE
    }
  ...
  override fun startNewDeviceTransferServer(context: Context, aep: AccountEntropyPool) {
    val pendingIntent = android.app.PendingIntent.getActivity(
      context,
      0,
      dev.chat.fork.messenger.MainActivity.clearTop(context),
    ...
    val notificationData = org.signal.devicetransfer.DeviceToDeviceTransferService.TransferNotificationData(
      dev.chat.fork.messenger.notifications.NotificationIds.DEVICE_TRANSFER,
      dev.chat.fork.messenger.notifications.NotificationChannels.getInstance().BACKUPS,
      org.signal.core.ui.R.drawable.ic_signal_backup
    )
    org.signal.devicetransfer.DeviceToDeviceTransferService.startServer(
      context,
      dev.chat.fork.messenger.devicetransfer.newdevice.NewDeviceServerTask(),
      notificationData,
      pendingIntent
    )
  }
