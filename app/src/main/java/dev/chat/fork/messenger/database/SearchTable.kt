...
import dev.chat.fork.messenger.jobs.RebuildMessageSearchIndexJob
...
class SearchTable(context: Context, databaseHelper: SignalDatabase) : DatabaseTable(context, databaseHelper) {
  ...
  fun queryMessages(query: String, filter: dev.chat.fork.messenger.search.SearchFilter): Cursor? {
    if (filter.isEmpty) {
